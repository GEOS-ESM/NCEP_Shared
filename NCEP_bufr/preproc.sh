...
do
  ...
  bnf=${bn}.f
  BNFS="$BNFS $bnf"
  cpp -P -C -D$byte_order -DNORMAL_BUILD -DSTATIC_ALLOCATION $i $bnf
done
...
cflags_defs="-DSTATIC_ALLOCATION"
...
cpp -P -C -DNORMAL_BUILD -DSTATIC_ALLOCATION bufrlib.PRM bufrlib.prm
