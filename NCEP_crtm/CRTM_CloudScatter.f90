...
MODULE CRTM_CloudScatter
  ...
  PUBLIC :: CRTM_Compute_CloudPhaseFnc
  PUBLIC :: CRTM_Compute_CloudPhaseFnc_TL
  PUBLIC :: CRTM_Compute_CloudPhaseFnc_AD
  ...
  RESULT( Error_Status )
    ...
    Cloud_loop: DO n = 1, Atm%n_Clouds
      ...
      Cloud_Layer_loop: DO k = 1, nCloud_Layers
        ...
        CScat%Asymmetry_Factor(kc) = CScat%Asymmetry_Factor(kc) + CSV%g(kc,n) * bs
...
!
! PURPOSE:
!       Function to compute the cloud particle scattering phase function and
!       populate the output CloudScatter structure for a single channel.
!
! CALLING SEQUENCE:
!       Error_Status = CRTM_Compute_CloudPhaseFnc( Atmosphere  , &
!                                                  SensorIndex , &
!                                                  ChannelIndex, &
!                                                  CloudScatter, &
!                                                  CSvar         )
...
!------------------------------------------------------------------------------
  ...
  FUNCTION CRTM_Compute_CloudPhaseFnc( &
    Atm         , &  ! Input
    SensorIndex , &  ! Input
    ChannelIndex, &  ! Input
    CScat       , &  ! Output
  ...
  RESULT( Error_Status )
    ...
    INTEGER :: Error_Status
    ! Function parameters
    CHARACTER(*), PARAMETER :: ROUTINE_NAME = 'CRTM_Compute_CloudPhaseFnc'
    ! Local variables
    CHARACTER(ML) :: Message
    INTEGER  :: k, kc, l, m, n
    REAL(fp) :: Frequency_MW, Frequency_IR
    ...
    IF (Atm%n_Clouds == 0 .OR. CScat%n_Legendre_Terms == 0) RETURN
...
!        CASE (THIRTYTWO_STREAMS); CScat%lOffset = ?
        CASE DEFAULT
          ...
          ELSE
            ...
            WRITE(Message,'("The n_Legendre_Terms in CloudPhaseFnc, ",i0,", do not fit model")') &
      ...
      Cloud_loop: DO n = 1, Atm%n_Clouds
        ...
        Cloud_Layer_loop: DO k = 1, nCloud_Layers
          ...
          ! Call sensor specific routines
          IF ( SpcCoeff_IsMicrowaveSensor(SC(SensorIndex)) ) THEN
            CALL Get_Cloud_PhaseFnc_MW(CScat                            , & ! Input
                                       Atm%Cloud(n)%Type                , & ! Input
          ...
          ELSE IF ( SpcCoeff_IsInfraredSensor(SC(SensorIndex)) .OR. &
                    SpcCoeff_IsVisibleSensor( SC(SensorIndex))      ) THEN
            ! IR and visible use the same cloud optical data file, but distingished with Frequency
            CALL Get_Cloud_PhaseFnc_IR(CScat                            , & ! Input
                                       Atm%Cloud(n)%Type                , & ! Input
                                       CSV%pcoeff(:,:,kc,n)             , & ! Output
                                       CSV%csi(kc,n)                      ) ! Interpolation
          ELSE
...
!          END IF
  ...
  END FUNCTION CRTM_Compute_CloudPhaseFnc
...
!
! PURPOSE:
!       Function to compute the tangent-linear cloud particle scattering 
!       phase function and populate the output CloudScatter_TL structure
!       for a single channel.
!
! CALLING SEQUENCE:
!       Error_Status = CRTM_Compute_CloudScatter_TL( Atmosphere     , &
...
!------------------------------------------------------------------------------
  ...
  FUNCTION CRTM_Compute_CloudPhaseFnc_TL( &
    Atm         , &  ! FWD Input
    CScat       , &  ! FWD Input
    Atm_TL      , &  ! TL  Input
    SensorIndex , &  ! Input
  ...
  RESULT( Error_Status )
    ...
    INTEGER :: Error_Status
    ! Local parameters
    CHARACTER(*), PARAMETER :: ROUTINE_NAME = 'CRTM_Compute_CloudPhaseFnc_TL'
    ! Local variables
    INTEGER  :: k, kc, l, m, n
    INTEGER  :: n_Legendre_Terms, n_Phase_Elements
    REAL(fp) :: Frequency_MW, Frequency_IR
    ...
    ! ------
    Error_Status = SUCCESS
    IF (Atm%n_Clouds == 0 .OR. CScat_TL%n_Legendre_Terms == 0) RETURN
    IF( CScat_TL%n_Phase_Elements > 0  .and. CScat%Include_Scattering ) THEN
!    IF( n_Phase_Elements > 0 .and. CScat%Include_Scattering ) THEN
      ...
      Cloud_loop: DO n = 1, Atm%n_Clouds
        ...
        Cloud_Layer_loop: DO k = 1, nCloud_Layers
          ...
          ! Call sensor specific routines
          IF ( SpcCoeff_IsMicrowaveSensor(SC(SensorIndex)) ) THEN
            CALL Get_Cloud_PhaseFnc_MW_TL(CScat_TL                            , & ! Input
                                          Atm%Cloud(n)%Type                   , & ! Input
                                          Atm_TL%Cloud(n)%Effective_Radius(kc), & ! TL  Input
                                          Atm_TL%Temperature(kc)              , & ! TL  Input
                                          ke_TL                               , & ! TL  Output
          ...
          ELSE IF ( SpcCoeff_IsInfraredSensor(SC(SensorIndex)) .OR. &
                    SpcCoeff_IsVisibleSensor( SC(SensorIndex))      ) THEN
            CALL Get_Cloud_PhaseFnc_IR_TL(CScat_TL                            , & ! Input
                                          Atm%Cloud(n)%Type                   , & ! Input
                                          Atm_TL%Cloud(n)%Effective_Radius(kc), & ! TL  Input
                                          ke_TL                               , & ! TL  Output
                                          w_TL                                , & ! TL  Output
...
!          END IF
      ...
      END DO Cloud_loop
    ELSE
!      CScat_TL%Phase_Coefficient(:,:,:) = ZERO    ! Is this correct?
    END IF

  END FUNCTION CRTM_Compute_CloudPhaseFnc_TL
...
!
! PURPOSE:
!       Function to compute the adjoint of the cloud scattering phase function
!       for a single channel.
!
! CALLING SEQUENCE:
!       Error_Status = CRTM_Compute_CloudPhaseFnc_AD( Atmosphere     , &
!                                                     CloudScatter   , &
!                                                     CloudScatter_AD, &
!                                                     SensorIndex    , &
!                                                     ChannelIndex   , &
...
!------------------------------------------------------------------------------
  ...
  FUNCTION CRTM_Compute_CloudPhaseFnc_AD( &
    Atm         , &  ! FWD Input
    CScat       , &  ! FWD Input
    CScat_AD    , &  ! AD  Input
    SensorIndex , &  ! Input
  ...
  RESULT( Error_Status )
    ...
    INTEGER :: Error_Status
    ! Local parameters
    CHARACTER(*), PARAMETER :: ROUTINE_NAME = 'CRTM_Compute_CloudPhaseFnc_AD'
    ! Local variables
    INTEGER  :: k, kc, l, m, n
    INTEGER  :: n_Legendre_Terms, n_Phase_Elements
    REAL(fp) :: Frequency_MW, Frequency_IR
    ...
    ! ------
    Error_Status = SUCCESS
    IF ( Atm%n_Clouds == 0 .OR. CScat_AD%n_Legendre_Terms == 0 ) RETURN
    IF( CScat_AD%n_Phase_Elements > 0 .and. CScat%Include_Scattering ) THEN 
      ...
      Cloud_loop: DO n = 1, Atm%n_Clouds
        ...
        Cloud_Layer_loop: DO k = nCloud_Layers, 1, -1 !1, nCloud_Layers
          ...
          ! Call sensor specific routines
          IF ( SpcCoeff_IsMicrowaveSensor(SC(SensorIndex)) ) THEN
            CALL Get_Cloud_PhaseFnc_MW_AD(CScat_AD                            , & ! Input
                                          Atm%Cloud(n)%Type                   , & ! Input
                                          ke_AD                               , & ! AD  Input
                                          w_AD                                , & ! AD  Input
                                          pcoeff_AD                           , & ! AD  Input
          ...
          ELSE IF ( SpcCoeff_IsInfraredSensor(SC(SensorIndex)) .OR. &
                    SpcCoeff_IsVisibleSensor( SC(SensorIndex))      ) THEN
            CALL Get_Cloud_PhaseFnc_IR_AD(CScat_AD                            , & ! Input
                                          Atm%Cloud(n)%Type                   , & ! Input
                                          ke_AD                               , & ! AD  Input
                                          w_AD                                , & ! AD  Input
                                          pcoeff_AD                           , & ! AD  Input
  ...
  END FUNCTION CRTM_Compute_CloudPhaseFnc_AD
...
!################################################################################
  ...
  SUBROUTINE Get_Cloud_Opt_IR( CloudScatter, &  ! Input  CloudScatter structure
    ...
    IF (.not. (CloudScatter%n_Phase_Elements > 0 .and. CloudScatter%Include_Scattering) ) THEN
  ...
  SUBROUTINE Get_Cloud_Opt_IR_TL( CloudScatter_TL, &  ! Input      CloudScatter TL structure
    ...
    IF ( .not. (CloudScatter_TL%n_Phase_Elements > 0 .and. CloudScatter_TL%Include_Scattering) ) THEN
      ! Absorption coefficient
      IF( w < ONE ) THEN
  ...
  SUBROUTINE Get_Cloud_Opt_IR_AD( CloudScatter_AD, &  ! Input      CloudScatter AD structure
    ...
    ! Perform interpolation
    ! ---------------------
    IF (.not. (CloudScatter_AD%n_Phase_Elements > 0 .and. CloudScatter_AD%Include_Scattering) ) THEN
      ! Absorption coefficient
      IF( w < ONE ) THEN
  ...
  SUBROUTINE Get_Cloud_Opt_MW( CloudScatter, &  ! Input  CloudScatter structure
    ...
    SELECT CASE (Cloud_Type)
      ...
      CASE (WATER_CLOUD)
        ...
        CALL interp_2D( CloudC%g_L_MW(csi%i1:csi%i2,j,csi%k1:csi%k2), csi%wlp, csi%ylp, g )
      ...
      CASE (ICE_CLOUD)
        ...
        CALL interp_1D( CloudC%g_S_MW(csi%i1:csi%i2,j,k), csi%wlp, g )
    ...
    SELECT CASE (Cloud_Type)
      ...
      CASE (WATER_CLOUD)
        ...
        CALL interp_2D_TL( z2   , csi%wlp, csi%ylp, &  ! FWD Input
                           ...
                           ke_TL                    )  ! TL  Output
        ! Asymmetry parameter
        z2 => CloudC%g_L_MW(csi%i1:csi%i2,j,csi%k1:csi%k2)
        CALL interp_2D_TL( z2   , csi%wlp, csi%ylp, &  ! FWD Input
                           z2_TL, wlp_TL , ylp_TL , &  ! TL  Input
                           g_TL                    )  ! TL  Output
  ...
  ! ------------------------------------------
  ! Subroutine to obtain the IR spherical 
  ! Legendre coefficients (pcoeff) for the 
  ! scattering phase function 
  ! ------------------------------------------
  SUBROUTINE Get_Cloud_PhaseFnc_IR( CloudScatter, &  ! Input  CloudScatter structure
                                    cloud_type  , &  ! Input  see CRTM_Cloud_Define.f90
  ...
  END SUBROUTINE Get_Cloud_PhaseFnc_IR

  ! ---------------------------------------------
  ! Subroutine to obtain the tangent-linear
  ! IR bulk optical properties of a cloud:
  !   extinction coefficient (ke_TL),
  !   scattereing coefficient (w_TL), and
  ! Legendre coefficients (pcoeff)
  ! ---------------------------------------------
  SUBROUTINE Get_Cloud_PhaseFnc_IR_TL( CloudScatter_TL, &  ! Input      CloudScatter TL structure
                                       cloud_type     , &  ! Input      see CRTM_Cloud_Define.f90
                                       Reff_TL        , &  ! TL  Input  effective radius (mm)
                                       ke_TL          , &  ! TL  Output extinction coefficient (=~ opt depth for 1 mm water content)
                                       w_TL           , &  ! TL  Output single scattering albedo
  ...
  END SUBROUTINE Get_Cloud_PhaseFnc_IR_TL

  ! ---------------------------------------------
  ! Subroutine to obtain the adjoint of the
  ! IR spherical Legendre coefficients
  ! ---------------------------------------------
  SUBROUTINE Get_Cloud_PhaseFnc_IR_AD( CloudScatter_AD, &  ! Input      CloudScatter AD structure
                                       cloud_type     , &  ! Input      see CRTM_Cloud_Define.f90
                                       ke_AD          , &  ! AD  Input  extinction coefficient 
                                                           !            (=~ optical depth for 1 mm water content)
                                       w_AD           , &  ! AD  Input  single scattering albedo
  ...
  END SUBROUTINE Get_Cloud_PhaseFnc_IR_AD

  ! ------------------------------------------
  ! Subroutine to obtain the MW spherical 
  ! Legendre coefficients (pcoeff) for the 
  ! scattering phase function 
  ! ------------------------------------------
  SUBROUTINE Get_Cloud_PhaseFnc_MW( CloudScatter, &  ! Input  CloudScatter structure
                                    cloud_type  , &  ! Input  see CRTM_Cloud_Define.f90
                                    pcoeff      , &  ! Output spherical Legendre coefficients
                                    csi           )  ! Output interpolation data
    ! Arguments
  ...
  END SUBROUTINE Get_Cloud_PhaseFnc_MW

  ! ---------------------------------------------
  ! Subroutine to obtain the tangent-linear
  ! MW cloud phase function
  ! ---------------------------------------------
  SUBROUTINE Get_Cloud_PhaseFnc_MW_TL( CloudScatter_TL, &  ! Input  CloudScatter TL structure
                                       cloud_type     , &  ! Input  see CRTM_Cloud_Define.f90
                                       Reff_TL        , &  ! TL  Input  effective radius (mm)
                                       Temperature_TL , &  ! TL  Input  cloudy temperature
                                       ke_TL          , &  ! TL  Output extinction coefficient (=~ opt depth for 1 mm water content)
  ...
  END SUBROUTINE Get_Cloud_PhaseFnc_MW_TL

  ! ---------------------------------------------
  ! Subroutine to obtain the adjoint of the
  ! MW cloud phase function
  ! ---------------------------------------------
  SUBROUTINE Get_Cloud_PhaseFnc_MW_AD(CloudScatter_AD, &  ! Input      CloudScatter AD structure
                                      cloud_type     , &  ! Input      see CRTM_Cloud_Define.f90 
                                      ke_AD          , &  ! AD  Input  extinction coefficient 
                                                          !            (=~ optical depth for 1 mm water content)
                                      w_AD           , &  ! AD  Input  single scattering albedo
  ...
  END SUBROUTINE Get_Cloud_PhaseFnc_MW_AD
